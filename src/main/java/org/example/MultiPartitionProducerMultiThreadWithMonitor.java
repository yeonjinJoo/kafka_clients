...
import org.example.monitor.writer.CsvSendRateMetricWriteStrategy;
import org.example.monitor.writer.MonitorLogWriter;
import org.example.monitor.writer.SendLogWriter;
...
public class MultiPartitionProducerMultiThreadWithMonitor implements Runnable {
    ...
    private String metricFilePath = "/Users/jooyeonjin/Desktop/GradProject_Apache_Kafka/kafka_clients/output/cur/sendRate_monitor.csv";


    private final MonitorQueue monitoringQueue = MonitorQueue.getInstance();

    private final Queue<Double> queue = new ConcurrentLinkedQueue<Double>();

    private MonitorLogWriter monitorLogWriter;

    private SendLogWriter sendLogWriter;

    private AtomicInteger ackCounter = new AtomicInteger();

    private IMessageAdaptor messageGenerator;

    private IMessageAdaptor messageGenerator2;

    private long absTimestampBase;

    public MultiPartitionProducerMultiThreadWithMonitor() {
        super();
    }
    ...
    public static void main(String[] args) {
        ...
        new CommandLine((new MultiPartitionProducerMultiThreadWithMonitor()))
                .execute(args);
    ...
    public void run() {
        ...
        Thread sendRateLogWriteThread = setupSendRateLogWriterThread();

        monitorLogWriteThread.start();
        sendRateLogWriteThread.start();
        ...
        sendLogWriter.gracefulShutdown();
        sendLogWriter.syncedNotify();

        try {
            sendRateLogWriteThread.join();
        } catch (InterruptedException e) {
            e.printStackTrace();
        }
    }
    ...
    private Thread setupSendRateLogWriterThread() {
        messageGenerator2 = new EfficientMessageGenerator(messageSize, 100_000);
        sendLogWriter = new SendLogWriter(
                new CsvSendRateMetricWriteStrategy(metricFilePath),
                messageGenerator2,
                monitorBatchSize,
                queue
        );
        return new Thread(sendLogWriter);
    }
    ...
    public class ProducerThread implements Runnable {
        ...
        public ProducerThread(Properties props, int partitionNum) {
            this.props = props;
        ...
        public void run() {
            try (Producer<String, String> producer = new KafkaProducer<>(props)) {
                ...
                producer.metrics().forEach((name, metric) -> {
                    // 전체 요청률(초당 요청 수)
                    if (name.group().equals("producer-metrics") && name.name().equals("request-rate")) {
                        Number n = (Number) metric.metricValue();
                        double rate = n.doubleValue();
                        queue.add(rate);
                        sendLogWriter.notifyIfNeeded();
                    }
                });
